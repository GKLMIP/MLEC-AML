...
def train_test_model(config, session, train_models, valid_models, test_models, trains, valids, tests, domain_classifier,
    ...
    for i in range(config.max_max_epoch):
        ...
        print('***************************')
        with open("emotion_chi20210618_gegeleibie_without_adversial_random_seed_1234.txt", 'a') as f:
        ...
        if total_acc/len(answers) > marco_f1_max:
            ...
            with open('chi_answer_20210618_without_adversial_random_seed_1234.json','w') as f:
                json.dump(answer_list,f)
            with open('chi_pred_20210618_without_adversial_random_seed_1234.json','w') as f:
...
if __name__ == "__main__":
    ...
    for domain in domain_list:
        train, valid, test = read_data.load_data(path='emotion_beishu_2'+config.dataset+'/'+domain+'/dataset',n_words=config.vocab_size, \
